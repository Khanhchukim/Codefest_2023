	...
	current_socket->on("ticktack player", sio::socket::event_listener_aux([&](string const& name, message::ptr const& data, bool isAck, message::list& ack_resp)
		{
			...
			auto currentPosition = players[player_id-1]->get_map()["currentPosition"]->get_map();
			...
			// update
			elBombGame->updateMap(map, bombs, power);
			elBombGame->updatePlayerPosition(currentPosition);

			
			// move
			std::string movePath = elBombGame->getPath();
			if (movePath != "") { 
				std::cout << "path: " << movePath << std::endl;
